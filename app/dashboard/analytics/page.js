...
export default function AnalyticsPage() {
  const { currentGym, isCoach } = useAuth();
  ...
  const fetchAllData = async () => {
    ...
    const [overviewResult, prsResult, participationResult, performersResult, athletesResult] =
      await Promise.all([
        getGymOverviewAction(currentGym.id),
        getRecentPRsAction(currentGym.id, 10),
        getParticipationStatsAction(currentGym.id, 7),
        getTopPerformersAction(currentGym.id, 5),
        getAthleteListAction(currentGym.id),
      ]);
