    ...
    try {
      for await (const ev of stream(`/api/workouts/${workout!.id}/enhance`, {
        ...
        if (ev.type === 'chunk') {
          ...
          setWorkout((w) => (w ? { ...w, body_detailed: body, generation_status: 'enhancing' } : w));
        } else if (ev.type === 'done') {
          setWorkout((w) =>
            w ? { ...w, body_detailed: body, generation_status: 'detailed' } : w
          );
  ...
  async function adapt() {
    ...
    try {
      for await (const ev of stream(`/api/workouts/${workout!.id}/adapt`, {
        ...
        if (ev.type === 'chunk') {
          body += ev.text as string;
          setWorkout((w) => (w ? { ...w, body_detailed: body } : w));
        } else if (ev.type === 'done') {
          setWorkout((w) =>
            w
              ? {
  ...
  return (
    <ScrollView className="flex-1 bg-bg" contentContainerClassName="p-4 gap-4">
      ...
      <View className="bg-zinc-950 border border-zinc-800 rounded-lg p-4">
        <Text className="text-fg font-mono text-sm leading-6">
