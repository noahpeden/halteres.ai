-- Migration: Fix handle_new_user trigger to include is_active column
-- The is_active column exists on profiles (NOT NULL) but was not included
-- in the trigger's INSERT, causing "Database error saving new user" on signup.
-- Also fixes: variable name shadowing type name, missing onboarding_completed for coaches.

-- First, ensure is_active column has a proper DEFAULT so existing code paths work
...
BEGIN
    ...
    IF v_role = 'athlete' AND v_gym_code IS NOT NULL AND v_gym_code != '' THEN
